...
export type UsePaginatedOptions<T, F = Record<string, unknown>> = {
  initialItems?: T[]; // optional local dataset
  pageSize?: number;
  fetchPage?: (page: number, perPage: number, opts?: F) => Promise<FetchPageResult<T>>; // optional remote fetcher
};

export function usePaginatedInstance<T, F = Record<string, unknown>>({ initialItems = [], pageSize = 12, fetchPage }: UsePaginatedOptions<T, F>) {
  const initialTotal = initialItems.length;
  ...
  const loadPage = useCallback(
    async (p: number, append = false, opts?: F) => {
      if (fetchPage) {
        setLoading(true);
        try {
    ...
    [fetchPage, pageSize, initialItems]
  );
  ...
  const loadMore = useCallback(() => {
    ...
    loadPage(next, true);
  }, [loadPage, page, pageSize, fetchPage, initialItems.length]);

  const reset = useCallback((opts: { page?: number; filters?: F } = {}) => {
    const p = opts.page ?? 1;
